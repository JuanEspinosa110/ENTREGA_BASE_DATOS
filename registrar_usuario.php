...
<?php
    if(isset($_POST['save'])) {
        ...
        $sql = $con->prepare("SELECT * FROM usuario WHERE doc_us = ?");
      ...
      echo "<script>alert('usuario ya existente');</script>";
        ...
        $sql = $con->prepare("INSERT INTO usuario (doc_us, nombre, celular, profesion, correo, contrasena, id_tip_us, descripcion) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $sql->execute([$documento, $nombre, $celular, $profesion, $correo, $contra, $rol, $desc_perfil]); 
...
<body>
    ...
    <nav class="navbar navbar-expand-lg bg-white navbar-light sticky-top py-0 pe-5">
        <a href="index.html" class="navbar-brand ps-5 me-0">
            <img src="img/logoSena.png" width="95" height="100">
        </a>
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <div class="form-outline mb-4">
                                    ...
                                    <select class="form-control" name="rol" id="rol" required>
                                        ...
                                        <?php
                                        $control = $con->prepare("SELECT * FROM tipo_us WHERE id_tip_us = 3");
                                        $control->execute();
                                        
                                        while($fila = $control->fetch(PDO::FETCH_ASSOC)) {
                                            echo '<option value=' . $fila['id_tip_us'].'>' . $fila['tip_us'] . '</option>';
                                        }
